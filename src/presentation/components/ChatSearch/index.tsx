...
const ChatSearch = () => {
  ...
  const {
    ...
    filesUploads,
    messagesQuerry,
    enterprise_ids: stateEnterpriseIds,
    uuid,
    country_ids,
    abonnementID,
  } = location.state || {};
  ...
  const { createIA, getIA, getPdf, getUserById, getCountryBySessionId } =
    UserSA();
  ...
  const [abonnementIdState, setAbonnementIdState] = useState<string | null>(
    abonnementID ?? null
  );
  ...
  const handleSendFristMessage = async (
    messages: string,
    files: File,
    filesName: string,
    enterprise_ids: string[],
    ...
    if (files) {
      setMessages((prev) => [
        ...
        {
          type: 'user',
          text: `📎 ${activeString.IA.DOCUMENT_SENT} : ${filesName}`,
        },
      ]);
  ...
  const handleSendMessage = async () => {
    ...
    if (!message && uploadedFile == null) return;
    if (!sessionId) return;
    if (uploadedFileName) {
      setMessages((prev) => [
        ...prev,
        {
          type: 'user',
          text: `📎 ${activeString.IA.DOCUMENT_SENT} : ${uploadedFileName}`,
        },
      ]);
      setUploadedFileName(null);
    } else if (message) {
  ...
  const formatBotMessage = (text: string) => {
    ...
    formatted = formatted.replace(
      /<!-- START: hidden metadata -->[\s\S]*?<!-- END: hidden metadata -->/g,
      '<br/><br/>'
    );
    ...
    formatted = formatted.replace(
      /(Score de correspondance\s*: :\s*[0-9]+\/[0-9]+)/g,
      '$1<br/>----------------------------'
    );
  ...
  const mapHistoryToMessages = (history: any[]) => {
    return history.map((item) => {
      const type =
        item.type === 'human' ? 'user' : item.type === 'ai' ? 'ai' : 'user';
  ...
  const handleGetIa = async () => {
    ...
    try {
      ...
      try {
        const countryResponse = await getCountryBySessionId(
          accessToken,
          sessionId
        );
  ...
  const isChatTypeAllowed = (): boolean => {
    ...
    const allowedChatTypes: Record<string, string[]> = {
      ...
      '6903247bd4a86732cb6f4f05': [
        '/chat/c/',
        '/chat/mixte/',
        '/chat/company/',
      ],
      '69032fe9d4a86732cb6f4f09': [
        '/chat/c/',
        '/chat/mixte/',
        '/chat/company/',
        '/chat/flag/',
    ...
    <>
      ...
      <div className="chatbot">
        ...
        <div className="chatbot__content">
          <div
            className={`chatbot__messages ${hasSentMessage ? 'height' : ''}`}
          >
            {flag && (
              <p>
              ...
              <div key={index} className={`message ${msg.type}`}>
                {msg.type === 'bot' || msg.type === 'ai' ? (
                  <>
                    ...
                    />
                    {msg.results != null && msg.results.length > 0 && (
          ...
          {isInitialLoading ? null : abonnementIdState &&
            isChatTypeAllowed() ? (
