...
const HistorySearchChat = () => {
    ...
    const [activeCompany, setActiveCompany] = useState<ActiveCompanyType>("default");
    ...
    const handleGetSessionId = async () => {
        try {
            ...
            return setSessionId(response.data)
        } catch (error) {
            console.warn(error)
        }
        finally {
    ...
    const handleNavigateSessionId = async (item: string) => {
        try {
            navigate(`/chat/${isCompany === 'default' ? 'c' : isCompany}/${item}`, { state: { abonnementID: abonnementID } });
        ...
        <div style={styles.containers}>
            ...
            <div style={{ marginTop: 24 }}>
                ...
                >
                    ...
                    <span style={{ color: activeCompany === "default" ? COLORS.white : COLORS.black }}>{activeString.HOME_COMPANY.WIPWORK_ONLY}</span>
                </button>
                <button className={`company-btn ${activeCompany === "company" ? "active" : ""}`} onClick={handleNavigateCompany} >
                    <img src={icons.base_de_donnees} alt="base de donnée" className="icon" />
                    <span style={{ color: activeCompany === "company" ? COLORS.white : COLORS.black }}>{activeString.HOME_COMPANY.CV_LIBRARY} </span>
                </button>
                <button
                    ...
                    <span style={{ color: activeCompany === "mixte" ? COLORS.white : COLORS.black }}>{activeString.HOME_COMPANY.COMBINED_SEARCH}</span>
                </button>
                <button
