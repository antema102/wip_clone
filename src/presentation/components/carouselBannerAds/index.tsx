...
const CarouselBannerAds = ({ isStyled = true }: CarouselBannerAdsProps) => {
  ...
  const getAllBanners = async () => {
    ...
    try {
      ...
      const responseBanner =
        await userSARef.current.getAdvertisementsByCategory(
          dataBanner,
          accessToken
        );
      ...
      if (items != null && items.length > 0) {
        ...
        if (firstLoadRef.current) {
          ...
          // @ts-expect-error setTimeout retourne number dans l'environnement RN/web
      ...
      if (err?.name === 'AbortError') {
        ...
        setError(
          err?.message ?? 'Erreur lors de la récupération des bannières'
        );
      }
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    // @ts-expect-error setInterval retourne number
