...
export const Confirmation = (props: any): any => {
  ...
  const resendCode = async () => {
    ...
    toast.current?.show({
      severity: 'success',
      summary: 'Success',
      detail: RESET_PASSWORD.RESEND_MESSAGE,
      life: 3000,
    });
    setTimeout(() => {
  ...
  const handleSubmit = async () => {
    if (otp.some((digit) => digit === '')) {
      toast.current?.show({
        severity: 'error',
        summary: 'Erreur',
        detail: 'Veuillez remplir le code confirmations',
        life: 3000,
      });
      return;
    }
                  ...
                  return (
                    <input
                      ...
                      onPaste={(e) => {
                        ...
                        for (
                          let j = 0;
                          j < pasteData.length && i + j < otp.length;
                          j++
                        ) {
              ...
              <div className="confirmation__btn">
                <button className="margin-bottom-8" onClick={handleSubmit}>
                  {isLoading ? (
                    ...
                    'Confirmer le code de validations'
                  )}
                </button>
                <button onClick={resendCode}>
                  {isLoadingResend ? (
                    ...
                    'Renvoyer le code de validations'
                  )}
                </button>
              </div>
              <div className="confirmation__ligne"></div>
            </div>
          </div>
        </div>
      </div>
