...
export const ForgetPassword = (props: any) => {
  ...
  const sendingEmail = async () => {
    if (emailValidation) {
      setIsLoading(true);
      resetUserPassword(email).then((response) => {
        if (response.status === HttpStatus.OK) {
          toast.current?.show({
            severity: 'success',
            summary: 'Success',
            detail: RESET_PASSWORD.USER_DOES_EXIST,
            life: 3000,
          });
          setEmail('');
          setIsLoading(false);
          ...
          toast.current?.show({
            severity: 'error',
            summary: 'Error',
            detail: RESET_PASSWORD.USER_DOES_NOT_EXIST,
            life: 3000,
          });
          setIsLoading(false);
        }
      ...
      toast.current?.show({
        severity: 'error',
        summary: 'Error',
        detail: RESET_PASSWORD.WRONG_FORMAT,
        life: 3000,
      });
      setIsLoading(false);
    }
    ...
    <>
      ...
      <div className="containerBackground">
        <div className="containers container">
          <CarouselContent
            text={
              'Bonjour et bienvenue dans le système de récupération de mot de passe. Veuillez entrer votre adresse e-mail associée à votre compte afin de procéder à la réinitialisation de votre mot de passe. Merci !'
            }
          />
                ...
                <div className="forgetPassword__btn">
                  <button onClick={sendingEmail}>
                    {isLoading ? (
                      ...
                      'Envoyer email de récuperations'
                    )}
                  </button>
                </div>
                <div className="forgetPassword__ligne"></div>
                <Footer
