...
const CandidatDetail = ({ stateValue }: any) => {
  ...
  const getUsers = async (receiverId: string) => {
    try {
      ...
      const { data: transmitterData } = await getUserById(
        user?.id,
        accessToken
      );
  ...
  return (
    <>
      <div style={styles.container}>
        <div
          style={{ overflowY: 'auto', ...{ backgroundColor: 'transparent' } }}
        >
          <Dialog
            animationType="slide"
            transparent
                ...
                {user?.role === 'company' && (
                  <div style={[styles.compatibilityItem]}>
                    ...
                    >
                      <img
                        src={
                          isFavorite ? icons.favoris : { uri: icons.deFavoris }
                        }
                        style={{
                          width: 28,
                          height: 26,
        ...
        <CustomModal
          ...
          content={
            <DisplayVideo
              ...
              viewOnly={user?.role !== ROLEACCOUNT.candidate}
            />
          }
        />
      ) : null}
