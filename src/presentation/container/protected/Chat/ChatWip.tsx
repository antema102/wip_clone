...
const ChatWip = () => {
    ...
    const { sendSessionId, getUserById, postCreateUuid, getExternalUSer, getPdf, updateCv } = UserSA();
    const [isLoading, setisLoading] = useState<boolean>(false);
    const [isThereprogressStatus, setIsThereProgressStatus] = useState<boolean>(false);
    ...
    const [isCvListModalVisible, setIsCvListModalVisible] = useState<boolean>(false);
    ...
    const handleCreateSessionID = useCallback(
        async (title: string, type: string, flag?: string) => {
            try {
                const response = await sendSessionId(accessToken, title, type == '' ? null : type, flag);
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            if (message) {
                ...
                if (enterprise_ids !== undefined) {
                    navigate(`/chat/${!type || type === "" ? "c" : type}/${sessionID}`, { state: stateData });
                } else {
            ...
            try {
                await axios.post(urls.POST_PDF, formData, {
                    ...
                    onUploadProgress: (progressEvent: ProgressEvent) => {
                        ...
                        const currentFileProgress = ((i + percent) / filesToUpload.length) * 100;
    ...
    const isCompanyAllowed = (companyType: string): boolean => {
        ...
        const allowedCompanies: { [key: string]: string[] } = {
    ...
    return (
        <>
            ...
            <Dialog
                ...
                onRequestClose={() => setIsCvListModalVisible(false)}
            >
                <div
                    className="cv-list-overlay"
                    onClick={() => setIsCvListModalVisible(false)}
                >
                    <div
                        ...
                        <div className="cv-list-content">
                            {isLoadingCvList ? (
                                <div className="cv-list-empty">
                                    ...
                                    <p className="cv-list-empty-text">{activeString.IA.LOADING}</p>
                                </div>
                                    ...
                                    <div key={index} className="cv-item">
                                        <div className="cv-item-info">
                                            ...
                                            <div className="cv-item-text">
                                                <p className="cv-item-name">{cv.last_name} {cv.first_name || `CV ${index + 1}`}</p>
                                                <p className="cv-item-date">
                                                    {cv.timestamp ? new Date(cv.timestamp).toLocaleDateString() : 'Date non disponible'}
                                                </p>
                                            </div>
                                        </div>
                                        <div className="cv-item-actions">
                                            <a
                                                ...
                                                title="Voir le CV"
                                                onClick={() => handleGetPDF(cv.storage_path)}
                                            >
                                                <img src={icons.eye} alt="Voir" height={24} style={{ cursor: 'pointer' }} />
                                            </a>
                                            <a href="#" className="cv-item-download" onClick={() => handleDeleteCV(cv.point_id)}>
                                                <img src={icons.deletes} alt="Supprimer" height={24} style={{ cursor: 'pointer' }} />
                                            </a>
                                        </div>
                                    </div>
                                ))
                                ...
                                <div className="cv-list-empty">
                                    <div className="cv-list-empty-icon">📭</div>
                                    <p className="cv-list-empty-text">{activeString.IA.CV_LIST_EMPTY}</p>
                                    <p className="cv-item-date">{activeString.IA.CV_LIST_EMPTY_SUBTITLE}</p>
                                </div>
                            )}
                        </div>
                ...
                >
                    ...
                    >
                        <div className="upload-modal-header">
                            ...
                            <h2 className="upload-modal-title">{activeString.IA.UPLOAD_CV}</h2>
                        ...
                        <div className="upload-modal-content">
                            {filesToUpload.length === 0 ? (
                                ...
                                <>
                                    ...
                                    <label htmlFor="file-upload-input" className="upload-drop-zone" style={{ marginTop: '16px', padding: '16px' }}>
                                        <div className="upload-drop-zone-icon" style={{ fontSize: '32px', marginBottom: '8px' }}>➕</div>
                                        <p className="upload-drop-zone-text" style={{ fontSize: '14px' }}>
                                            {activeString.IA.ADD_MORE_FILES || 'Ajouter plus de fichiers'}
                                        </p>
                                        <input
                                            id="file-upload-input"
                                            type="file"
                        ...
                        !isCompanyAllowed(company) ? (
                            <div className="chat__intro">
                                <div className="subscription-notice">
                                    <span className="subscription-notice__icon" role="img" aria-label="Cadenas">🔒</span>
                            ...
                            <>
                                {(!company || company === '') ? (
                                    <div className="chat__intro">
                                        <p className="chat__intro-title">
                                            {activeString.IA.TITLE}
                                        </p>
                                        <p className="chat__intro-subtitle">{activeString.IA.SUB_TITLE}</p>
                                    </div>
                                    ...
                                    <div className="chat__intro">
                                        ...
                                        <div className="upload-cv-section">
                                            <button className="button__cv" onClick={() => setIsUploadModalVisible(true)}>
                                        ...
                                        <div className="upload-cv-section">
                                            <button onClick={handleGetExternalUser} className="button__cv button__cv--view">
