...
export const EntResult = (props: any) => {
  ...
  const getDataUser = async () => {
    try {
      ...
      const { data }: any = await getUserById(
  ...
  const getOfferJobById = async () => {
    setIsLoading(true);
    try {
      const response = await allOfferJobByEnt(
        accessToken,
        stateValue?.companyId
      );
        ...
        <>
          <div style={[styles.containers, styles.contents]}>
            <div style={styles.containerAvatar}>
              <img src={avatar || images.avatar_6} style={styles.images} />
              <div style={{ gap: 14 }}>
                <div>
                  <span style={styles.textAvatar}>{profil.name}</span>
                </div>
                <div>
                  <span style={{ fontSize: 18 }}>{profil.activity}</span>
                </div>
              </div>
            </div>
            ...
            <div style={styles.textContent}>
              <ItemResult
                label={activeString.ENTERPRISE_OFFER.CREATION}
                text={profil.yearOfCreation}
              />
              <ItemResult
                label={activeString.ENTERPRISE_OFFER.LIEU}
                text={profil.headQuarter}
              />
              <ItemResult
                label={activeString.ENTERPRISE_OFFER.EMAIL}
                text={profil.email}
              />
              {profil.url && <ItemResult label="Site web" text={profil.url} />}
            </div>
          </div>
          ...
          <div
            style={{
              ...
              ...(isMobile
                ? {}
                : { paddingVertical: 24, paddingHorizontal: 70 }),
            }}
          >
            ...
            <Popup email={profil?.email} web={profil?.url} />
          </div>
        </>
      )}
