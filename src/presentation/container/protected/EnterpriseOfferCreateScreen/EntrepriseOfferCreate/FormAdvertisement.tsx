...
import {
  getDynamicEchelleByKey,
  getDynamicListByKey,
} from '../../../../../service/technique/dynamicService';
...
export const FormAdvertisement = (props: any) => {
  ...
  const { createAdvertisement, editAdvertisement, removeAdvertisement } =
    UserSA();
  ...
  const handleSubmit = async () => {
    ...
    if (condition) {
      ...
      if (modiferData) {
        ...
        response = await editAdvertisement(
          modiferData.id,
          formData,
          accessToken
        );
  ...
  return (
    ...
    >
        ...
        <div style={{ paddingVertical: 20 }}>
          <FileUploader
            handleFileChange={async (e) => {
              await selectImage(e, false);
            }}
            fileName={videoName || 'Importer'}
          />
        </div>
      )}
      {imageError && (
      ...
      {imageName !== '' ? (
        ...
        >
          <img
            src={`data:image/jpg;base64,${imageName}`}
