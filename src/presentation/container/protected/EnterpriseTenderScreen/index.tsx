...
export const EnterpriseTender = (props: any) => {
  ...
  const { getAllTenders, getCostsUser, getTenderPDF, deleteTenderById } =
    UserSA();
  ...
  const getAllTendersOfThisUser = async () => {
    ...
    let response;
    if (!responseData || newData || index !== 1) {
  ...
  const renderItemTemplate = (item: any) => (
    <Fragment>
      <ItemTender
        ...
        isDelete={true}
        actionDelete={async () => {
          await activatePopupDeletion(item?.id);
        }}
      />
    </Fragment>
  );
      ...
      <div style={styles.container}>
        ...
        {refreshing ? (
          ...
          <>
            ...
            >
              <TitleRefont
                title={activeString.DETAIL_PROFIL.HEADER_TENDER_TITLE}
              />
              <div style={styles.containers}>
                {data?.length ? (
                  ...
                  <NotFounds
                    label={activeString.DETAIL_PROFIL.NOTFOUND_APPEL_OFFERS}
                    image={icons.cardProfil}
                    customImage={{ height: 125, width: 125 }}
                  />
                )}
                <FixedButtonCircle
        ...
        <Popup
          ...
          btnTitle="OK"
          action={async () => {
            await deleteTender(itemToDelete);
          }}
          cancel={true}
        />
      </div>
